...
static void decon_esd_enable_interrupt(struct decon_device *decon)
{
	...
	if (esd) {
		...
		if (esd->det_irq) {
			decon_info("%s: det(%d) is %s\n", __func__, esd->det_gpio, gpio_get_value(esd->det_gpio) ? "high" : "low");
			enable_irq(esd->det_irq);
		}
	}
}
...
irqreturn_t decon_pcd_handler(int irq, void *dev_id)
{
	struct esd_protect *esd;
...
irqreturn_t decon_err_handler(int irq, void *dev_id)
{
	struct esd_protect *esd;
	struct decon_device *decon = (struct decon_device *)dev_id;
	...
	if (esd->esd_wq && !esd->queuework_pending) {
...
irqreturn_t decon_det_handler(int irq, void *dev_id)
{
	struct esd_protect *esd;
	struct decon_device *decon = (struct decon_device *)dev_id;
	...
	esd = &decon->esd;
	level = gpio_get_value(esd->det_gpio);
...
static int decon_register_esd_funcion(struct decon_device *decon)
{
	...
	if (gpio_is_valid(gpio)) {
		...
		if (esd->err_pin_active == gpio_get_value(esd->det_gpio)) {
			decon_info("%s: det(%d) is already %s(%d)\n", __func__, esd->det_gpio,
				(esd->det_pin_active) ? "high" : "low", gpio_get_value(esd->det_gpio));
		}
	}
	...
	if (esd->pcd_irq) {
		if (devm_request_irq(dev, esd->pcd_irq, decon_pcd_handler,
	...
	if (esd->det_irq) {
		...
		disable_irq_nosync(esd->det_irq);
	}
...
decon_init_done:
	...
	if (dsim) {
		if (!lcdtype) {
			decon_err("decon does not found panel\n");
			decon->ignore_vsync = true;
		}
